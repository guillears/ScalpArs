...
- **Dual-review gate (Sep-8, operator-locked):** EVERY commit runs ① caveman review (terse findings pass) AND ② deep review (thorough verification pass) before the diff is shown; findings from both applied pre-commit. No exceptions for 'simple' changes.
...
- **🔒 COHORT COMPLETENESS (Sep-25): when sizing a rule that EXTENDS or RE-SCOPES an existing gate, add back the fills that gate already screened out** — the pool's BASE era is built from SCREENED_BASELINE, so screened fills are invisible there (use the COMBINED raw pool). The heat re-scope evidence was 9 trades until 3 screened BASE longs (2 winners) were found. Sleeve studies start from the sleeve's pin in CURRENT_STATE (e.g. the MOMENTUM-LONG ANALYSIS PIN). Rationale: DECISION_LOG (116).
...
- **"High WR but net-losing" is NOT a filter candidate *when the loss is sizing*.** A 60–75% WR cohort can be net-negative purely from a few multiplier-amplified fat-tail losers → fix sizing on the specific losers; don't block the winning cohort. Always de-multiply to 1× before judging; only a cohort still below the sleeve's breakeven WR with negative 1× expectancy (the expectancy bar below) is a filter candidate.
...
- **🔒 OBSERVE-FIRST RULE (Aug-16, from the FAKE_BULL_GUARD refutation): entry-filter candidates below the locked gates ship OBSERVE-ONLY** — the signature is tracked on real full-size fills at batch reviews (entry columns record it for free); ARMED deployment only after the observed cohort meets the locked filter gates. Armed overrides stay reserved for live-bleed cases (fade-SL class) where waiting has a real daily cost.
- **🔒 WINDOW-UNITS RULE (Aug-16, operator-caught): any filter candidate whose conditions include MARKET-WIDE variables (regime/breadth/BTC state) is a sleeve on/off switch, not a trade filter — evidence counts in WINDOW units (same-window blocks = ONE observation), and per-window concentration is checked like per-pair concentration.** FAKE_BULL_GUARD's "N=10" was ~7 windows; it never met any gate.
- **🔒 EXIT/STOP COUNTERFACTUALS are read on the LIVE-STOPPED cohort only (Sep-23, second stop-width miss).** A replica re-sim stops trades live never stopped (intrabar low-before-high hits wicks live polling rode through); every wider-stop variant then "rescues" them and the whole-cohort Δ turns positive on pure artefact. Split the CF into saved (live-stopped, survives) vs deeper (live-stopped, stops wider anyway); fills live did not stop must show Δ=0 or the replica is lying. Two-sided, per era, day-block bootstrap CI, then decide. Rationale: DECISION_LOG (110).
...
- **Pattern C → FILTER (block entry) — EXPECTANCY bar (Sep-25, operator: "the goal is a profitable platform, not break-even"; replaces the old N≥30 ∧ WR≤40% ∧ avg≤−0.20% ∧ never-positive≥60% bar, which only caught disasters):** judge the blocked cohort at 1× (de-multiplied) on ALL of: ① **WR below the sleeve's breakeven WR** = |avg loss| / (avg win + |avg loss|), computed per sleeve on current-stack kept fills (momentum long ≈61% at Sep-25) · ② **avg P&L% < 0 with ≥95% confidence** by a WINDOW-CLUSTERED bootstrap (resample market windows, not trades) · ③ **≥8 distinct windows** and no single window/pair carrying ≥50% of the cohort loss · ④ **N ≥ 15**. Then the 30–50% in-sample haircut and a pre-committed tight revert gate. A cohort that fails only on confidence stays a pre-registered observe candidate (frozen threshold) — never re-fit the threshold on the data that failed it. Rationale: DECISION_LOG (115).
