...
- Full filter list = `trading_config.json`. **Pair blacklist = 10 pairs (trimmed from 23 on 2026-06-03; BTCUSDT re-whitelisted same day).**
  ...
  - **🔓 RELEASED / WHITELISTED from blacklist (13, on 2026-06-03):** `ADAUSDT, ASTERUSDT, BCHUSDT, BTCUSDT, DOGEUSDT, ERAUSDT, HYPEUSDT, ICPUSDT, LABUSDT, LINKUSDT, PUMPUSDT, SKYAIUSDT, WLFIUSDT`. 12 released for thin pre-filter evidence (N<15). **BTCUSDT released per user direction AGAINST the evidence** (structural low-vol loser: 26% WR / −0.21% avg / N=27, edge ~0.19% ≈ 0.077% fee) — accepted as a deliberate override; current filters block ~83% of BTC trades so it rarely fires. **REVERT GATE: re-blacklist any released pair (incl. BTC) at ≤35% WR on N≥10 fresh.** BTC near-certain to trip it if it trades meaningfully — watch closely.
