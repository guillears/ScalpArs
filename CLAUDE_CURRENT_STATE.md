...
- **ATR-HIGH multiplier (LONG) = 2.0×inv (shipped 2026-06-05):** entry_atr >**1.1%** → ×2 (the "runner cohort" — high-ATR longs reach the trailing arm ~80% vs ~30%, all 6 RUNNER_TRAIL longs ATR≥1.0). Dimensional candidate (max-wins), **pattern-blocked** (keeps 2× off C-pattern/DOA high-ATR longs like INJ), hard-capped 2×. Near-neutral on the batch (doubles INJ/STO losers ≈ +$13 net) — operator-directed, tight revert gate below. Source label `ATR_HI`.
...
- **ATR-HIGH 2× LONG (Jun 5, in-sample ship — tight gate):** ATR>1.1 → ×2, pattern-blocked, hard-capped. Near-neutral on the batch (doubles INJ/STO losers; ≈ +$13 net). **REVERT to 1.0× if Total$<0 on N≥5 fresh ATR>1.1 longs.** Watch the Multiplier Cell / `ATR_HI` source rows.
