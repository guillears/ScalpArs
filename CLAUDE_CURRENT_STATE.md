...
- Full filter list = `trading_config.json`. **Pair blacklist = 11 pairs (trimmed from 23 on 2026-06-03).**
  - **BLACKLISTED (11):** `BNBUSDT, BTCUSDT, ENAUSDT, FILUSDT, MUSDT, RAVEUSDT, TRUMPUSDT, VVVUSDT, XAGUSDT, XAUUSDT, ZECUSDT`. Of these: 5 evidenced losers (BNB/BTC/FIL/TRUMP/VVV, N≥15) + XAG/XAU (commodities) + ENA/MU/RAVE/ZEC (no-data, new-listing/illiquid). BTCUSDT blacklist (tradeable) is separate from BTC-as-macro-reference (still computed).
  - **🔓 RELEASED / WHITELISTED from blacklist (12, on 2026-06-03):** `ADAUSDT, ASTERUSDT, BCHUSDT, DOGEUSDT, ERAUSDT, HYPEUSDT, ICPUSDT, LABUSDT, LINKUSDT, PUMPUSDT, SKYAIUSDT, WLFIUSDT`. Reason: all were blacklisted on thin pre-filter evidence (N<15, e.g. LABUSDT N=1, ADAUSDT −0.033 ~breakeven) — now re-tested forward under the current 15+ filter stack. **REVERT GATE: re-blacklist any released pair that shows ≤35% WR on N≥10 fresh (current-stack) trades.** Watch per-pair WR as fresh trades accumulate.
