...
class SignalThresholds(BaseModel):
    ...
    bull_long_fan_max: float = 5.0                     # upper fan bound (Jun 23: 3.0→5.0 — operator widened the revival test to the FULL range via UI to collect complete live data across all fan buckets. Includes the ≥3.0 confirmed-loser tail; acceptable at 1× obs for the full picture. Safe = 1× obs.)
    bull_long_fan_min: float = 0.85                    # lower fan bound (Jun 23: 1.35→0.85 — operator widened the revival test via UI to collect data across the whole fan range incl. the <1.35 band [confirmed 25%WR loser]; cheap at 1× obs. 0 = disabled)
    ...
    bull_long_lev_mult: float = 0.05                   # leverage multiplier. Jun 22: 0.05/1×→1.0/20× RE-LEVERED (operator override, N=8/88% fresh-reset). Jun 23: DE-LEVERED 1.0→0.05 (1× obs) — the INSTANT revert gate TRIPPED on the very next batch: 7 bull-longs/29%WR/-$335 all H.BULL, 4 of first 6 hit SL AND two gapped past -1.0% (TNSR -1.19, ALLO -1.01); the fan 1.65-2.0 sub-band inverted exactly as the cross-batch warning said (5/20%/-$264). Pre-committed gate, automatic. Back to 1× observation until a cross-batch-STABLE re-lever case exists. TIGHT REVERT history kept: re-lever only at N≥30 cumulative + WR≥70%/avg≥+0.15% across ≥2 batches.
