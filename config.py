...
class SignalThresholds(BaseModel):
    ...
    bull_long_fan_max: float = 3.0                     # upper fan bound (Jun 23: 2.0→3.0 for the revival test — widen to the full plausible band; cross-batch N=86 shows 2.0-3.0 ≈ 1.65-2.0 [both ~+1.7%], so no reason to exclude 2-3. Excludes only ≥3.0 [confirmed loser]. Safe at 1× obs.)
    bull_long_fan_min: float = 1.35                    # lower fan bound (Jun 22: 1.0→1.35 — operator restricts LIVE band to fan 1.35-2.0, the sweet spot; bull-only this band = 8/88%WR/+0.554% [the 1.0-1.35 band lost: 5/60%/−$96]. 0 = disabled
