...
class SignalThresholds(BaseModel):
    ...
    flip_fan_qs_cell: str = "3:70:60-90:1.0:1.0"  # qs_min : bear_min : range_lo-range_hi : size_mult : lev_mult — the FAN flip-SHORT "winner cell" (quality_score ≥ qs_min AND entry_bear_pct ≥ bear_min AND range_lo ≤ entry_range_position ≤ range_hi). Jun 26: REPLACED the old BTC-RSI×ADX mult cell (flip_fan_mult_rule, retired — it was never used at 1×). Default 1.0:1.0 = INERT (no sizing change), shipped only to TRACK the cell as its own row in 💰 Multiplier Cell Performance: in-sample canonical 72-pool N=14/85.7%WR/+0.463%/+$671 (clears the W-gate on WR/avg/$, fails only N=14<30); out-of-sample 06-26 fired once +$36. ⚠ BE-compat FAILS for a 2× (0/2 cell losers peaked ≥+0.20%) → if ever sized up, 1.5× first per staging, never straight to 2×. SHORT flips only (a flip-LONG ignores it, sizes 1×). Empty = off. → DECISION_LOG/CURRENT_STATE 2026-06-26.
