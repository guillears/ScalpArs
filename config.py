...
class SignalThresholds(BaseModel):
    ...
    bull_long_fan_min: float = 0.85                    # lower fan bound (Jun 23: 1.35→0.85 — operator widened the revival test via UI to collect data across the whole fan range incl. the <1.35 band [confirmed 25%WR loser]; cheap at 1× obs. 0 = disabled). NOTE: when bull_long_fan_by_regime has an entry for the gated regime, that per-regime [min,max] OVERRIDES these globals; these remain the fallback for any allowed regime NOT in the map.
