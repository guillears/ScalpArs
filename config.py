...
class SignalThresholds(BaseModel):
    ...
    bull_long_lev_mult: float = 1.0                    # leverage multiplier (Jun 22: 0.05/1×→1.0/20× RE-LEVERED — operator discipline-override, tested on a FRESH RESET batch. ⚠ N=8 bull-only < N≥30 gate + 20× jump skips the 1.5× stage. Justified by: fresh-reset = clean attribution, band edge strong this batch (88%/+0.55%). TIGHT REVERT: de-lever→0.05 at live N≥10 NEW bull-longs if fan 1.35-2.0 S+H-bull WR≤70% OR avg≤+0.15%; INSTANT de-lever if 3 of first 6 hit SL OR any single gaps past ~−1.0%)
