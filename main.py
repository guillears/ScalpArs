    ...
    try:
        ...
        for p in _ps:
            ...
            rows.append({
                ...
                'blk_sp': p.blocked_spacing or 0, 'blk_sl': p.blocked_slots or 0, 'blk_ema': p.blocked_ema50 or 0, 'blk_24h': getattr(p, 'blocked_off24h', 0), 'blk_e13': getattr(p, 'blocked_ema13', 0) or 0, 'blk_pvr': getattr(p, 'blocked_pvr', 0) or 0, 'blk_1h': getattr(p, 'blocked_1h', 0) or 0, 'blk_br': getattr(p, 'blocked_breadth', 0) or 0, 'blk_age': getattr(p, 'blocked_age', 0) or 0,
...
async def _compute_performance(db: AsyncSession, regime: str = None, window_hours: int = None,
    ...
    try:
        ...
        entry_funnel = {
            ...
            # sub-label matches the "had room" headline (SUPP excluded from both).
            "blocked_long": _fb.get('total_long_room', _fb.get('total_long', 0)),
            "blocked_short": _fb.get('total_short_room', _fb.get('total_short', 0)),
    ...
    return {
        ...
        # pattern_truly_unmatched RETIRED Jul-28 (operator): with keep-only-unmatched ON the cohort is
...
# _compute_truly_unmatched RETIRED Jul-28 (operator table-value review): under keep-only-unmatched
# the no-C/no-W cohort duplicated the Unmatched Losers table row-for-row; aggregate lives on in
# _compute_pattern_cohort_coverage's 'truly_unmatched' row.
...
def _compute_runner_trail_performance(orders):
    ...
    # Jul-28 compression (operator table-value review): the decision surface is the
    # Leash Shadow aggregate + this summary line (both computed over the FULL cohort);
    # the per-trade listing is capped to the worst 10 + best 5 by gainΔ$ so the report
    # keeps the forensic tails without 69 near-identical mid rows.
    if len(rows) > 15:
        by_gain = sorted(rows, key=lambda r: r['gain_usd'])
        keep = by_gain[:10] + by_gain[-5:]
        keep_ids = {id(r) for r in keep}
        rows = [r for r in rows if id(r) in keep_ids]  # preserve chronological order
        if summary:
