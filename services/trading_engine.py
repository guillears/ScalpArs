...
class TradingEngine:
    ...
    async def initialize(self, db: AsyncSession):
        ...
        # May 14 — One-time auto-backfill of entry_btc_dist_from_ema13_pct on historical Orders.
        ...
        # Remove this line + the method below in a follow-up commit after verification.
        asyncio.create_task(self._backannotate_btc_dist_from_ema13())
        self._initialized = True

    async def _backannotate_btc_dist_from_ema13(self):
        """Auto-backfill `entry_btc_dist_from_ema13_pct` on historical Orders.

        Idempotent — only updates rows where the field is currently NULL.

        Fetches BTC 5m OHLCV from Binance (1500 candles = ~5.2 days), computes
        EMA13, then for each Order with NULL entry_btc_dist_from_ema13_pct,
        looks up the candle that contains opened_at and computes
        dist_pct = (close - ema13) / ema13 * 100.
        Approximation note: live capture uses realtime intra-candle BTC price;
        backfill uses the 5m candle close. Discrepancy = 0-5min of price drift,
        small relative to typical bucket widths (0.10%).
        """
        try:
            from database import AsyncSessionLocal
            ...
            # Brief wait so the bot's main loop is up.
            ...
            async with AsyncSessionLocal() as db:
                _q = await db.execute(
                    sa_select(Order).where(
                        Order.status == 'CLOSED',
                        Order.entry_btc_dist_from_ema13_pct.is_(None),
                    ).limit(1)
                )
                if _q.scalar_one_or_none() is None:
                    logger.info('[BTC_DIST_EMA13_BACKFILL] No orders need backfill — skipping.')
                    return

            logger.info('[BTC_DIST_EMA13_BACKFILL] Starting auto-backfill of entry_btc_dist_from_ema13_pct on historical orders...')

            # Fetch 1500 5m candles (~5.2 days). Older trades will be flagged too_old.
            try:
                ohlcv = await binance_service.get_ohlcv('BTC/USDT:USDT', '5m', 1500)
            except Exception as e:
                logger.error(f'[BTC_DIST_EMA13_BACKFILL] Failed to fetch BTC 5m OHLCV: {e}')
                return

            if not ohlcv or len(ohlcv) < 20:
                logger.warning(f'[BTC_DIST_EMA13_BACKFILL] Insufficient OHLCV ({len(ohlcv) if ohlcv else 0} candles). Aborting.')
                return

            df = pd.DataFrame(ohlcv, columns=['ts', 'open', 'high', 'low', 'close', 'volume'])
            df['ts'] = pd.to_datetime(df['ts'], unit='ms')
            df['ema13'] = df['close'].ewm(span=13, adjust=False).mean()
            df = df.dropna(subset=['ema13']).reset_index(drop=True)
            if df.empty:
                logger.warning('[BTC_DIST_EMA13_BACKFILL] EMA13 series empty after dropna. Aborting.')
                return

            earliest = df['ts'].iloc[0].to_pydatetime()
            latest = df['ts'].iloc[-1].to_pydatetime()
            logger.info(f'[BTC_DIST_EMA13_BACKFILL] EMA13 series covers {earliest} → {latest} ({len(df)} candles)')
            ...
            async with AsyncSessionLocal() as db:
                ...
                for o in orders:
                    ...
                    o.entry_btc_dist_from_ema13_pct = round(dist_pct, 4)
                    updated += 1
                await db.commit()
                logger.info(
                    f'[BTC_DIST_EMA13_BACKFILL] Done. Updated={updated}, too_old={too_old}, no_open_ts={no_open_ts}'
                )
        except Exception as e:
            logger.error(f'[BTC_DIST_EMA13_BACKFILL] Backfill failed: {e}')
