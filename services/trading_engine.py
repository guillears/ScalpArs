...
class TradingEngine:
    ...
    async def initialize(self, db: AsyncSession):
        ...
        # May 14 — One-time auto-backfill of analytics dimensions.
        # Covers entry_btc_ema20_slope (BTC 5m slope) + entry_dist_from_ema13_pct (pair EMA13 distance).
        # Idempotent — only fills NULL rows. Remove this line + the method in a follow-up
        # commit once verified, same pattern as e5d5625 / fd910ba.
        asyncio.create_task(self._backannotate_extension_dimensions())
        self._initialized = True

    async def _backannotate_extension_dimensions(self):
        """One-shot startup backfill for analytics dimensions.

        Fields backfilled:
        - entry_btc_ema20_slope: BTC 5m EMA20 slope % (used by 5m × 1h Slope Alignment cross-tab)
        - entry_dist_from_ema13_pct: pair price's distance from its own 5m EMA13 %
          (used by DOUBLE-STRETCH cross-tab)

        NOT backfillable here:
        - entry_global_volume_ratio: live aggregate across ALL scanned pairs;
          can't be reconstructed without replaying full scan history.

        Idempotent: only updates NULL rows. Logs counts per field.
        """
        try:
            from database import AsyncSessionLocal
            ...
            await asyncio.sleep(8)  # brief delay to let startup settle

            # Quick guard: anything to do?
            async with AsyncSessionLocal() as db:
                _q = await db.execute(
                    sa_select(Order).where(
                        Order.status == 'CLOSED',
                        sa_or(
                            Order.entry_btc_ema20_slope.is_(None),
                            Order.entry_dist_from_ema13_pct.is_(None),
                        ),
                    ).limit(1)
                )
                if _q.scalar_one_or_none() is None:
                    logger.info('[EXT_BACKFILL] No orders need backfill — skipping.')
                    return

            logger.info('[EXT_BACKFILL] Starting analytics-dimension backfill...')

            # ─────────────────────────────────────────────────────────
            # PHASE 1: BTC 5m slope (entry_btc_ema20_slope)
            # ─────────────────────────────────────────────────────────
            btc_updated = 0
            btc_too_old = 0
            try:
                ohlcv = await binance_service.get_ohlcv('BTC/USDT:USDT', '5m', 1500)
                if ohlcv and len(ohlcv) >= 25:
                    ...
                    earliest = df['ts'].iloc[0].to_pydatetime() if not df.empty else None
                    logger.info(f'[EXT_BACKFILL] BTC 5m series: {len(df)} candles, earliest={earliest}')
                    ...
                    if not df.empty:
                        async with AsyncSessionLocal() as db:
                            ...
                            for o in orders:
                                ...
                                o.entry_btc_ema20_slope = round(slope, 4)
                                btc_updated += 1
                            await db.commit()
                else:
                    logger.warning('[EXT_BACKFILL] BTC 5m OHLCV insufficient, skipping BTC slope phase.')
            except Exception as e:
                logger.error(f'[EXT_BACKFILL] BTC slope phase failed: {e}')

            logger.info(f'[EXT_BACKFILL] BTC slope: updated={btc_updated}, too_old={btc_too_old}')

            # ─────────────────────────────────────────────────────────
            # PHASE 2: Pair 5m EMA13 distance (entry_dist_from_ema13_pct)
            # ─────────────────────────────────────────────────────────
            pair_updated = 0
            pair_too_old = 0
            pair_fetch_fail = 0
            ...
            async with AsyncSessionLocal() as db:
                ...
                orders_needing_pair = result.scalars().all()

            # Group by pair
            from collections import defaultdict
            by_pair = defaultdict(list)
            for o in orders_needing_pair:
                if o.pair and o.opened_at:
                    by_pair[o.pair].append(o)

            logger.info(f'[EXT_BACKFILL] Pair phase: {sum(len(v) for v in by_pair.values())} orders across {len(by_pair)} distinct pairs')

            for pair_idx, (pair_sym, orders_list) in enumerate(by_pair.items()):
                # Format pair for CCXT (BTCUSDT → BTC/USDT:USDT)
                if pair_sym.endswith('USDT'):
                    base = pair_sym[:-4]
                    ccxt_symbol = f'{base}/USDT:USDT'
                else:
                    ccxt_symbol = pair_sym
                try:
                    pair_ohlcv = await binance_service.get_ohlcv(ccxt_symbol, '5m', 1500)
                except Exception as e:
                    logger.warning(f'[EXT_BACKFILL] {pair_sym}: fetch failed: {e}')
                    pair_fetch_fail += len(orders_list)
                    continue

                if not pair_ohlcv or len(pair_ohlcv) < 15:
                    pair_fetch_fail += len(orders_list)
                    continue

                pdf = pd.DataFrame(pair_ohlcv, columns=['ts', 'open', 'high', 'low', 'close', 'volume'])
                pdf['ts'] = pd.to_datetime(pdf['ts'], unit='ms')
                pdf['ema13'] = pdf['close'].ewm(span=13, adjust=False).mean()
                pdf = pdf.dropna(subset=['ema13']).reset_index(drop=True)
                if pdf.empty:
                    pair_fetch_fail += len(orders_list)
                    continue

                p_earliest = pdf['ts'].iloc[0].to_pydatetime()

                async with AsyncSessionLocal() as db:
                    for o in orders_list:
                        # Re-fetch the order in this session to attach it for update
                        result = await db.execute(
                            sa_select(Order).where(Order.id == o.id)
                        )
                        order = result.scalar_one_or_none()
                        if order is None or order.entry_dist_from_ema13_pct is not None:
                            continue
                        opened = order.opened_at
                        if opened.tzinfo is not None:
                            opened = opened.replace(tzinfo=None)
                        if opened < p_earliest:
                            pair_too_old += 1
                            continue
                        mask = pdf['ts'] <= opened
                        if not mask.any():
                            pair_too_old += 1
                            continue
                        row = pdf.loc[mask].iloc[-1]
                        close_val = float(row['close'])
                        ema13_val = float(row['ema13'])
                        if ema13_val == 0:
                            continue
                        dist_pct = (close_val - ema13_val) / ema13_val * 100.0
                        order.entry_dist_from_ema13_pct = round(dist_pct, 4)
                        pair_updated += 1
                    await db.commit()

                pairs_processed += 1
                # Light rate limiting between pairs
                if pair_idx + 1 < len(by_pair):
                    await asyncio.sleep(0.25)

            logger.info(
                f'[EXT_BACKFILL] Pair dist: updated={pair_updated}, too_old={pair_too_old}, '
                f'fetch_fail={pair_fetch_fail}, pairs_processed={pairs_processed}/{len(by_pair)}'
            )
            logger.info('[EXT_BACKFILL] Done.')
        except Exception as e:
            logger.error(f'[EXT_BACKFILL] Top-level failure: {e}')
